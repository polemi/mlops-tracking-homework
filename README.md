...
В этом репозитории выполнено ДЗ по MLOps и трекингу экспериментов.

Что сделано:

- обучена базовая модель ResNet18 на `Train_1`;
- качество базовой модели проверено на `Test_1`;
- метрики обучения и тестирования залогированы в TensorBoard;
- обученная модель загружена в локальный MinIO S3;
- модель дообучена на `Train_2`;
- дообученная версия проверена на `Test_2` и также загружена в S3;
- собран DVC-пайплайн для обучения, дообучения и сравнения моделей.
